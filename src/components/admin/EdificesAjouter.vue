      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="text-2xl sm:text-3xl font-light text-gray-900">
            ...
            <span v-else>Ajouter</span> un édifice religieux
          </h1>
          <p class="text-sm sm:text-base text-gray-500 mt-1">
            <span v-if="isEditing">Modifiez les informations de l'édifice</span>
            <span v-else>Remplissez le formulaire pour ajouter un nouvel édifice religieux</span>
          </p>
        </div>
        <RouterLink 
          to="/admin/edifices"
    ...
    <form @submit.prevent="handleSubmit" class="bg-white rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg border border-gray-100">
        ...
        <div class="space-y-6">
          ...
          <div>
            <label for="name" class="block text-sm font-medium text-gray-700 mb-2">
              Nom de l'édifice <span class="text-red-500">*</span>
            </label>
            <input 
              id="name"
              v-model="form.name"
          ...
          <div>
            ...
            <textarea 
              ...
              placeholder="Décrivez l'édifice religieux..."
            ></textarea>
          </div>
          ...
          <div>
            ...
            <input 
              ...
              required
              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all duration-300 outline-none"
              placeholder="Ex: Grand-Bassam, Côte d'Ivoire"
            />
          </div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-8 pt-6 border-t border-gray-100">
        ...
        >
          ...
          <span v-else>Enregistrer l'édifice</span>
        </button>
...
const handleFileUpload = (event: Event) => {
  ...
  // Réinitialiser l'input pour permettre de ré-uploader les mêmes fichiers
  target.value = ''
}
...
const handleSubmit = async () => {
  // Validation simple
  if (!form.name || !form.description || !form.location || !form.status) {
  ...
  alert(isEditing.value ? '✅ Édifice mis à jour avec succès !' : '✅ Édifice ajouté avec succès !')
  
  // Rediriger vers la liste
  router.push('/admin/edifices')
}
...
onMounted(() => {
  ...
  if (id) {
    ...
    form.name = 'Cathédrale Saint-Esprit'
    form.description = 'Une cathédrale magnifique...'
    form.location = 'Grand-Bassam, Côte d\'Ivoire'
    form.year = '1996'
    form.status = 'en-cours'
    form.features = ['Architecture gothique', 'Vitraux colorés', 'Jardin paysager']
  }
})
</script>
